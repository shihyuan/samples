#ifndef _MOD_MSGS_UTIL_HPP
#define _MOD_MSGS_UTIL_HPP
...
bool backInterpolate(const PoseVec& posVec, const ros::Time& stamp, PoseVec::const_reverse_iterator& rIt, mod_msgs::Pose2DStamped& pose2DStamped)
{
...
bool frontInterpolate(const PoseVec& posVec, const ros::Time& stamp, PoseVec::const_iterator& it, mod_msgs::Pose2DStamped& pose2DStamped)
{
...
mod_msgs::PedTraj downsample(const mod_msgs::PedTraj& ped_traj, ros::Duration duration, bool from_last = true)
{ //Downsample a pedTraj message according to duration, starting from the latest point and use interpolation.
    if (duration <= ros::Duration(0.0)){
        return ped_traj;
    }
    mod_msgs::PedTraj ped_traj_out;
    ped_traj_out = ped_traj;
    ped_traj_out.traj = downsample(ped_traj.traj,duration,from_last);
    return ped_traj_out;
}

mod_msgs::PedTrajVec downsample(const mod_msgs::PedTrajVec& pedTrajVec,ros::Duration duration, bool from_last = true)
{
    if (duration <= ros::Duration(0.0)){
        return pedTrajVec;
    }
    mod_msgs::PedTrajVec pedTrajVec_out;
    std::vector<mod_msgs::PedTraj>::const_iterator it;
